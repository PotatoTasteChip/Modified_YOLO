...
output_folder_fn = "TP_FP_FN_Classifier/V11_0_401/FN"
...
class_fn_counts = {class_name: 0 for class_name in class_names.values()}
...
for image_path in image_files:
    ...
    for g_idx, gt in enumerate(gt_objects):
        if g_idx not in matched_gt:
            ...
            class_fn_counts[class_name] += 1

            # FN 저장 폴더 생성
            class_folder_fn = os.path.join(output_folder_fn, class_name)
            os.makedirs(class_folder_fn, exist_ok=True)

            # FN 이미지 저장
            fn_img = img.copy()
            x1, y1, x2, y2 = map(int, gt['bbox'])
            cv2.rectangle(fn_img, (x1, y1), (x2, y2), (255, 0, 0), 2)
            cv2.putText(fn_img, f"FN:{class_name}", (x1, y1 - 5),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 1)
            fn_filename = filename.replace(".jpg", f"_{class_name}_FN.jpg")
            cv2.imwrite(os.path.join(class_folder_fn, fn_filename), fn_img)

# FN 개수 기록
for class_name, fn_count in class_fn_counts.items():
    with open(os.path.join(output_folder_fn, f"{class_name}_FN_count.txt"), 'w') as f:
        f.write(f"{fn_count}\n")
